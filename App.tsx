...
const App: React.FC = () => {
  ...
  return (
    <div className="relative w-full h-screen flex flex-col items-center justify-center bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white overflow-hidden">
      <audio ref={audioRef} src={PIANO_MUSIC_URL} loop preload="auto" />
      <ParticleBackground />

      {/* Intro Screen */}
      {appState === AppState.INTRO && (
        <div className="z-10 text-center animate-pulse cursor-pointer" onClick={handleStart}>
      ...
      {(appState === AppState.PLAYING || appState === AppState.FINISHED) && wishes[currentIndex] && (
        <div 
          className="z-10 max-w-4xl px-8 text-center transition-opacity duration-[1500ms] ease-in-out"
